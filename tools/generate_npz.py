...
# python generate_npz.py --gpu 0,1 --cfg ../experiments/cfgs/faster_rcnn_end2end_resnet.yml --def ../models/vg/ResNet-101/faster_rcnn_end2end_final/test.prototxt --out ../trainval2014_resnet101_faster_rcnn_genome_scores.tsv --net ../data/faster_rcnn_models/resnet101_faster_rcnn_final_iter_320000.caffemodel --split coco_trainval
...
FIELDNAMES = ['image_id', 'image_w', 'image_h', 'num_boxes', 'boxes', 'features', 'scores', 'attr_scores']
...
def get_detections_from_im(net, im_file, image_id, conf_thresh=0.2):
    ...
    return {
        ...
        'scores': base64.b64encode(scores[keep_boxes]),
        'attr_scores': base64.b64encode(attr_scores[keep_boxes])
    }
...
def generate_npz(gpu_id, prototxt, weights, image_ids, outfile):
    # First check if file exists, and if it is complete
    wanted_ids = set([int(image_id[1]) for image_id in image_ids])
    found_ids = set()
    # if os.path.exists(outfile):
    ...
    if len(missing) > 0:
        ...
        for im_file, image_id in image_ids:
            if int(image_id) in missing:
                ...
                if os.path.exists(os.path.join('../trainval', str(image_id)+'.npz')):
                    continue

                t_image_id, t_image_w, t_image_h, t_num_boxes, t_boxes, t_features, t_scores, t_attr_scores = \
                    get_detections_from_im(net, im_file, image_id)

                np.savez(os.path.join('../trainval', str(image_id)+'.npz'),
                         image_id=t_image_id,
                         image_w=t_image_w,
                         image_h=t_image_h,
                         num_boxes=t_num_boxes,
                         boxes=t_boxes,
                         features=t_features,
                         scores=t_scores,
                         attr_scores=t_attr_scores)
